...
let toastError = (response: object) => {
  let err
  if (!response.data) {
    err = "服务器返回错误"
  } else {
  ...
  response => {
    ...
    if (response.status === 200 && (response.data.ReturnCode === "000000" || response.data._RejCode === "000000" || response.data.ReturnCode === "P047" || response.data._RejCode === "P047")) {
      ...
      if (response.data === "" || response.data.jsonError || response.data.ReturnCode || response.data._RejCode) {
        if (response.config.isToast) { toastError(response) }
...
// 封装post请求
export function fetch(requestUrl: string, params = {}, method: string = 'GET', isLoading: boolean = true, isToast: boolean = true, loadingText: string) {
  let url: string = baseUrl + requestUrl //基础路径
